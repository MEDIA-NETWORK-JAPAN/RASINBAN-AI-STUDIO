...
| ID | 画面名 | URL | 説明 | モック |
|----|--------|-----|------|--------|
| G01 | ログイン | `/login` | 全ユーザー共通のログイン画面（管理者は二段階認証へ） | [G01_login.html](../mocks/G01_login.html) |
| G02 | 二段階認証 | `/two-factor-challenge` | 管理者ログイン時のOTPコード入力画面 | [G02_two_factor_auth.html](../mocks/G02_two_factor_auth.html) |
...
[G01 ログイン]
    │
    ├─→ (is_admin=true) [G02 二段階認証] ──→ [A01 ダッシュボード] ←────────────────┐
    │                                              │                                │
    │                                              ├─→ [A02 拠点一覧] ──→ [A03 拠点編集]
    │                                              │       │
    │                                              │       └─→ [A04 CSV一括登録]
    ...
    │                                              └─→ [A09 災害復旧エクスポート]
    │
    └─→ (is_admin=false) [U01 拠点ダッシュボード] (一般ユーザー、閲覧のみ)
```
