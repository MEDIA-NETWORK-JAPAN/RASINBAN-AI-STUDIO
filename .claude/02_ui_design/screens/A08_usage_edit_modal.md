...
API利用回数の誤りを手動で補正するためのモーダル。課金に関わる重要なデータ操作のため、Before/Afterの比較や理由の記録を重視。対象はユーザー単位（`monthly_api_usages.user_id`）。
...
|------|------|
| 対象年月 | `year_month` |
| 契約プラン (ユーザー) | `user.plan.name` + 上限値 |
| ユーザー (User) | `user.name`（拠点名 `team.name` をサブテキスト表示） |
