...
| Slug変更 | 警告メッセージ表示「Slugを変更すると既存の連携に影響があります」 |
| 保存ボタン | DifyApp更新 → トースト表示 |
| APIキー表示 | マスク解除（復号表示） |
| APIキー更新 | 新しいキーを暗号化保存 |
| 削除ボタン | 確認モーダル表示 |
| 削除確認 | DifyAppをソフトデリート（deleted_atをセット）→ A05へ遷移 |

**注記:**
- アプリ削除はソフトデリート（論理削除）で実装され、物理削除は行われません
- 削除されたアプリに関連する`monthly_api_usages`データは保持され、監査証跡・課金データとして利用できます
- 削除されたアプリはデフォルトの一覧には表示されませんが、必要に応じて復元可能です
